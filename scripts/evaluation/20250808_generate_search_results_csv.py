...
@click.option(
    ...
    default=None,
    type=str,
    help="Comma-separated list of original IDs to process (if not specified, all records will be processed)",
)
def main(input_path: Path, output_path: Path, tags: str, top_k: int, search_size: int, original_ids: str):
    ...
    # Filter data by original_ids if specified
    if original_ids:
        # Parse comma-separated IDs and convert to appropriate type
        target_ids = [id_str.strip() for id_str in original_ids.split(",") if id_str.strip()]
        logger.info(f"Filtering data for original IDs: {target_ids}")

        # Try to match IDs (handle both string and numeric IDs)
        if "ID" in df.columns:
            # Convert target_ids to match the type of ID column
            id_column = df["ID"]
            if id_column.dtype in ["int64", "float64"]:
                # Try to convert to numeric if possible
                try:
                    target_ids = [int(id_str) if id_str.isdigit() else id_str for id_str in target_ids]
                except ValueError:
                    pass  # Keep as strings if conversion fails

            df = df[df["ID"].isin(target_ids)]
            logger.info(f"Filtered to {len(df)} rows matching specified IDs")
        else:
            logger.warning("ID column not found in input data. Processing all rows.")

    if len(df) == 0:
        logger.error("No rows to process after filtering. Exiting.")
        return
