"""LLM execution and recovery helpers for PFMEA mapping jobs."""
...
def execute_mapping_job(
    ...
    with time_block(
        ...
        try:
            base_response = _run_mapping_with_rate_limit_retry(
            ...
            result = _recover_from_malformed_response(
    ...
    result = _recover_missing_requirements(
...
def _run_mapping_with_rate_limit_retry(
    executor: LLMExecutor,
    ...
    for attempt in range(1, _RATE_LIMIT_POLICY.max_attempts + 1):
        try:
            ...
            response = map_functions_to_requirements(
                executor.model,
                request,
                executor=executor,
            ...
            time.sleep(_MALFORMED_POLICY.base_delay)
            continue
...
def _recover_from_malformed_response(
    ...
    *,
    ...
    for start in range(0, total, chunk_size):
        ...
        try:
            chunk_response = map_functions_to_requirements(
                executor.model,
                chunk_request,
                executor=executor,
...
def _recover_missing_requirements(
    executor: LLMExecutor,
    request: FunctionMappingRequest,
    context: MappingContext,
    ...
    try:
        retry_response = map_functions_to_requirements(
            executor.model,
            retry_request,
            executor=executor,
            context=retry_context,
...
__all__ = ["execute_mapping_job", "handle_parallel_error"]
