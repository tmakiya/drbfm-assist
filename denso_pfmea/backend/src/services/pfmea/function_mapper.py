...
async def amap_functions_to_requirements(
    model: Any,
    ...
    *,
    ...
    """Map functions to requirements using LLM (async version).
    ...
    runner = executor or LLMExecutor(model, operation_name="llm_function_mapping")
...
def _parse_mapping_payload(
    ...
    *,
    ...
    try:
        ...
        logger.error(
            "PFMEA function mapping 応答のJSON解析に失敗: %s",
            exc,
            exc_info=True,
        )
        raise MalformedMappingResponseError(
            "AI応答をJSONとして解析できませんでした。",
            raw_text=prepared,
            original=exc,
        ) from exc
