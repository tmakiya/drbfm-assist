"""Cleaner version of PFMEA mapping runtime using Context Object pattern (async version).
...
from .mapping_execution import aexecute_mapping_job, handle_parallel_error
...
async def aensure_function_mappings(
    session_manager: Any,
    ...
    *,
    ...
    """Ensure PFMEA contexts have function⇔requirement mappings (async version).
...
async def _aprocess_jobs_parallel(
    ...
    """Process mapping jobs in parallel with Circuit Breaker protection (async version)."""
    ...
    async def _execute_job(
        job: MappingJob, index: int
    ) -> tuple[MappingJob, FunctionMappingResponse]:
        nonlocal completed_count
        async with semaphore:
            try:
                response = await aexecute_mapping_job(executor, job)
                completed_count += 1
                return (job, response)
            except Exception as exc:
                # Re-raise with job context for error handling
    ...
    with time_block(
        ...
        tasks = [_execute_job(job, i) for i, job in enumerate(jobs)]
        results = await asyncio.gather(*tasks, return_exceptions=True)

    # Check for errors
    for i, res in enumerate(results):
        if isinstance(res, BaseException):
            # Create a ParallelExecutionError-like exception for handle_parallel_error
            from src.common.concurrency import ParallelExecutionError

            parallel_exc = ParallelExecutionError(
                i, jobs[i], f"並列タスクが失敗しました: {res}"
            )
            parallel_exc.__cause__ = res
            handle_parallel_error(parallel_exc)

    # Apply results
    with time_block(
        "pfmea.mapping.apply_results",
        metadata={"phase": "pfmea", "jobs": len(results)},
    ):
...
__all__ = ["aensure_function_mappings"]
