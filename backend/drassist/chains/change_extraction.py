...
class ChangeExtractionState(BaseGraphState):
    ...
    change_points: List[ChangePoint] = Field(
        default_factory=list, description="Extracted change points"
    )


def extract_change_points(
    state: ChangeExtractionState, gemini_client: GeminiClient, langsmith_client
) -> Dict[str, Any]:
    ...
    langsmith_prompt = langsmith_client.pull_prompt("Decompose change points")
...
if __name__ == "__main__":
    ...
    # Example state
    initial_state = ChangeExtractionState(
        query="eBTMのレイアウトおよび制御の改良によりエア抜き作業性を向上"
    )
