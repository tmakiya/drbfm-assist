...
class DefectEstimation(BaseModel):
    ...
    potential_cause: str = Field(
        ..., description="Potential cause of the predicted defect"
    )
    potential_defect: str = Field(
        ..., description="Description of the anticipated risk"
    )
    countermeasure: str = Field(
        ..., description="Corresponding countermeasure for the risk"
    )
...
class EstimateDefectsState(BaseGraphState):
    ...
    search_results: List[Dict[str, Any]] = Field(
        ..., description="Search results from DRBFM workflow"
    )
    ...
    final_results: List[Dict[str, Any]] = Field(
        default_factory=list, description="Integrated final results"
    )
...
def estimate_defects_for_each_result(
    ...
    logger.info(
        f"Starting defect estimation for {len(state.search_results)} search results"
    )

    # Get prompt and response schema from LangSmith
    langsmith_prompt = langsmith_client.pull_prompt(
        "estimate_defects_and_countermeasures"
    )
    response_schema = langsmith_prompt.schema_["response_schema"]
    system_instruction = langsmith_prompt.messages[0].content

    def process_single_result(
        search_result: Dict[str, Any],
    ) -> tuple[str, DefectEstimation]:
    ...
    logger.info(
        f"Completed estimation with {len(estimation_results)} total estimations"
    )
