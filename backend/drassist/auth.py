...
async def authenticate(headers: dict) -> Auth.types.MinimalUserDict:
    ...
    if not auth_header:
        logger.error("No authorization header provided", available_headers=list(headers.keys()))
    ...
    if not tenant_id:
        ...
        raise Auth.exceptions.HTTPException(status_code=401, detail="Missing tenant_id in token")
...
async def filter_threads_by_tenant(
    ctx: Auth.types.AuthContext, value: Auth.types.on.threads.search.value  # noqa: ARG001
) -> Auth.types.FilterType:
