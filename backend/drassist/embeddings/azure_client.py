    ...
    def generate_embedding(
        ...
        for attempt in range(retry_count):
            try:
                ...
                self.logger.error(
                    "Authentication or request error (not retrying)", error=str(e)
                )
                return None
        ...
        for attempt in range(retry_count):
            try:
                ...
                if attempt < retry_count - 1:
                    ...
                    self.logger.error(
                        "Failed to generate batch embeddings after retries",
                        retry_count=retry_count,
                    )
                ...
                self.logger.error("Unexpected error generating batch embeddings", error=str(e))
                return [None] * len(texts)
