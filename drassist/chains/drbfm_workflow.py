...
class DrbfmWorkflowState(BaseGraphState):
    ...
    part: Optional[str] = Field(..., description="Optional part input for enhanced search")
...
def extract_attributes_from_query(
    ...
    langfuse_prompt = langfuse_client.get_prompt("Extract attributes from query")
...
def _extract_search_results(response: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for hit in response.get("hits", {}).get("hits", []):
        result = {
            ...
            "project_number": hit["_source"].get("project_number", ""),
            "part": hit["_source"].get("part", ""),
            "model_number": hit["_source"].get("model_number", ""),
            "title": hit["_source"].get("title", ""),
            "content": hit["_source"].get("content", ""),
            "cause": hit["_source"].get("cause", ""),
            "countermeasure": hit["_source"].get("countermeasure", ""),
            "recurrence_prevention": hit["_source"].get("recurrence_prevention", ""),
...
def execute_keyword_search_with_attributes(
    ...
    filters = build_field_filters(
        field_filters,
        model_numbers=model_numbers,
        exclude_model_numbers=exclude_model_numbers,
    )
    ...
    response = es_manager.search(query, size=ctx.search_size)
...
def evaluate_search_relevance(
    ...
    relevance_prompt = langfuse_client.get_prompt("decide_whether_change_re-trigger_past_cause")
    ...
    needs_additional_search = len(state.relevant_search_results) + positive_count < ctx.top_k
...
def determine_next_search_stage(state: DrbfmWorkflowState, runtime: Runtime[DrbfmWorkflowContext]) -> str:
    ...
    if state.needs_additional_search:
        ...
        logger.info(f"Sufficient relevant results found: {total_relevant} >= {ctx.top_k} threshold")
...
class DrbfmWorkflow(BaseGraph):
    ...
    def create_workflow(self) -> StateGraph:
        ...
        # Load model numbers for the initial search phase
        initial_model_numbers = []
        if self.product_segment:
            key = f"model_numbers.{self.product_segment}"
            initial_model_numbers = self.config_manager.get(key, [])
            if not initial_model_numbers:
                logger.warning(
                    f"No model numbers found for product segment: '{self.product_segment}' using key: '{key}'"
                )
            else:
                logger.info(
                    f"Loaded {len(initial_model_numbers)} model numbers for initial search (segment: {self.product_segment})"
                )
        else:
            # If no segment is specified, all models are included in the initial search,
            # so the expanded search phase will have no models to search.
            all_model_numbers_config = self.config_manager.get("model_numbers", {})
            if isinstance(all_model_numbers_config, dict):
                for segment, models in all_model_numbers_config.items():
                    if isinstance(models, list):
                        initial_model_numbers.extend(models)
                logger.info(
                    f"No product segment specified. Loaded {len(initial_model_numbers)} model numbers for initial search."
                )
            else:
                logger.warning(
                    "`model_numbers` in config is not a dictionary. No model number filter will be applied."
                )
        ...
        workflow.add_node(
            ...
            partial(
                ...
                model_numbers=initial_model_numbers,
                search_stage=1,
            ),
        )
        workflow.add_node("evaluate_search_relevance_with_attributes_stage_1", evaluate_relevance_func)
        ...
        workflow.add_node("evaluate_search_relevance_with_attributes_stage_2", evaluate_relevance_func)
        ...
        workflow.add_node("evaluate_search_relevance_with_attributes_stage_3", evaluate_relevance_func)

        # --- Expanded Search (Stages 4-6) with exclusion ---

        # with attributes search stage_4
        workflow.add_node(
            "execute_keyword_search_with_attributes_stage_4",
            partial(
                execute_keyword_search_with_attributes,
                es_manager=self.es_manager,
                match_type="match_phrase",
                exclude_model_numbers=initial_model_numbers,
                search_stage=4,
            ),
        )
        workflow.add_node("evaluate_search_relevance_with_attributes_stage_4", evaluate_relevance_func)

        # with attributes search stage_5
        workflow.add_node(
            "execute_keyword_search_with_attributes_stage_5",
            partial(
                execute_keyword_search_with_attributes,
                es_manager=self.es_manager,
                match_type="match",
        ...
        workflow.add_node("evaluate_search_relevance_with_attributes_stage_5", evaluate_relevance_func)

        # with attributes search stage_6
        workflow.add_node(
            "execute_embedding_search_with_attributes_stage_6",
            partial(
                execute_embedding_search_with_attributes,
                es_manager=self.es_manager,
                embedder=self.embedder,
        ...
        workflow.add_node("evaluate_search_relevance_with_attributes_stage_6", evaluate_relevance_func)
        ...
        def determine_whether_to_expand_search(state: DrbfmWorkflowState) -> str:
            """Determine if the search should be expanded to other product segments."""
            if state.needs_additional_search and self.product_segment:
                logger.info("Expanding search to other product segments (exclusion).")
                return "expand_search"
            else:
                logger.info("Sufficient results found or no segment specified. Proceeding to format.")
                return "format_results"

        # Start and initial search decision
        ...
        # Initial search phase (Stages 1-3)
        workflow.add_edge(
            "execute_keyword_search_with_attributes_stage_1",
        ...
        # Decision point: expand search or format results
        workflow.add_conditional_edges(
            "evaluate_search_relevance_with_attributes_stage_3",
            determine_whether_to_expand_search,
            {
                "expand_search": "execute_keyword_search_with_attributes_stage_4",
                "format_results": "format_results",
            },
        )

        # Expanded search phase (Stages 4-6)
        workflow.add_edge(
            "execute_keyword_search_with_attributes_stage_4",
            "evaluate_search_relevance_with_attributes_stage_4",
        )
        workflow.add_conditional_edges(
            "evaluate_search_relevance_with_attributes_stage_4",
            determine_next_search_stage,
            {
                "next_search_stage": "execute_keyword_search_with_attributes_stage_5",
                "format_results": "format_results",
            },
        )
        workflow.add_edge(
            "execute_keyword_search_with_attributes_stage_5",
            "evaluate_search_relevance_with_attributes_stage_5",
        )
        workflow.add_conditional_edges(
            "evaluate_search_relevance_with_attributes_stage_5",
            determine_next_search_stage,
            {
                "next_search_stage": "execute_embedding_search_with_attributes_stage_6",
                "format_results": "format_results",
            },
        )
        workflow.add_edge(
            "execute_embedding_search_with_attributes_stage_6",
            "evaluate_search_relevance_with_attributes_stage_6",
        )

        # Final path to formatting and end
        workflow.add_edge("evaluate_search_relevance_with_attributes_stage_6", "format_results")
