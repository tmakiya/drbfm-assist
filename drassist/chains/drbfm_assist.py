...
from drassist.chains.drbfm_workflow import DrbfmWorkflow, DrbfmWorkflowContext, DrbfmWorkflowState
...
class DrbfmAssistWorkflowState(BaseGraphState):
    ...
    # Input
    raw_input: str = Field(..., description="Raw input text containing multiple change points")
    part: Optional[str] = Field(None, description="Optional part input for enhanced search")

    # Intermediate
    change_points: List[str] = Field(default_factory=list, description="List of extracted change points")

    # Results aggregation using reducer
    per_cp_results: Annotated[List[Dict[str, Any]], add] = Field(
        default_factory=list, description="Aggregated results from each change point processing"
    )
...
def split_change_points(
    ...
    # Get Langfuse prompt for decomposing change points
...
def fanout(state: DrbfmAssistWorkflowState, runtime: Runtime[DrbfmAssistWorkflowContext]) -> List[Send]:
...
def format_batch_results(
    ...
    logger.info(f"Batch processing completed: {successful_results}/{total_results} successful")
...
class DrbfmAssistWorkflow(BaseGraph):
    ...
    def drbfm_workflow(self) -> DrbfmWorkflow:
        ...
        if self._drbfm_workflow is None:
            self._drbfm_workflow = DrbfmWorkflow(
                ...
                gemini_model_name=self._gemini_model_name,
    ...
    def create_workflow(self) -> StateGraph:
        ...
        workflow = StateGraph(DrbfmAssistWorkflowState)
