...
def get_workflow(product_segment: str = None):
    ...
    return DrbfmAssistWorkflow(
        config_path="configs/6307204b.yaml", 
        ...
        for cp_result in per_cp_results:
            ...
            if not search_results:
                ...
                for search_result in search_results:
                    ...
                    aqos_url = f"http://tadanogt116.tadano.co.jp/aqos/TR0210.aspx?KANRI_NO={kanri_number}&FUGOU={fugou}"
...
st.title("AIリスク・対策抽出 for Tadano")
...
with st.sidebar:
    ...
    config_path = Path(__file__).parent / "configs/6307204b.yaml"
    ...
    for i, item in enumerate(st.session_state.change_items):
        with st.container():
            ...
            st.session_state.change_items[i]["part"] = col1.text_input(
                "項目", value=item["part"], key=f"part_{item['id']}"
            )
            st.session_state.change_items[i]["raw_input"] = col2.text_area(
                "変更内容", value=item["raw_input"], key=f"raw_input_{item['id']}"
            )
            if i > 0:
